...
| Global processing pause is operational state with one durable authority (`AppSettings` has no `paused`; nothing writes it back; `/stats`, pause/resume results and the frontend `processingPaused` projection all derive from `TransferRepository.globally_paused()`; a legacy config value is a one-shot migration input) | `test_processing_pause_authority.py`, `test_application_runtime.py`, `test_v106_corrective_regressions.py`, `test_webhook_settings_integration.py` |
| `NormalizedError.recovery` / `operator_action_required` are output-only projections: `policy.recovery_action` is a pure function of canonical facts, no production code reads the fields back, no emitter supplies them, and no policy decision changes when they are set to any value | `test_recovery_projection_boundary.py` |
| Pattern-recurrence guards: no browser-side status derivation beside the backend presentation, no locally adjusted server-derived count, exactly one renderer for the Details file rows, no wrapper/proxy around the canonical owner, static shell mounts filled (never created or reparented) by their owners | `test_pattern_recurrence_contract.py`, `test_ui_shell_contract.py`, `test_settings_extraction_ui.py`, `test_consolidation_events.py` |
| Canonical equivalence/lifecycle correction (proof-attempt budget is not identity truth; unresolved equivalence blocks writer admission; same-transfer durable mapping via candidate binding/origin provenance; a satisfied logical delivery obligation cannot vote FAILED) — see `UNIVERSAL_TRANSFER_CORE.md` | `test_equivalence_retry_remediation.py`, `test_multi_mirror_general_http_convergence.py`, `test_workspace4_cohort_exit_gate.py`, `test_operational_artifact_membership.py`, `test_cross_transfer_equivalence.py`, `test_candidate_provenance_consolidation.py` |
| Canonical lifecycle/recovery/completion rework (transfer 265: one parent-lifecycle owner with no post-aggregate override; every recovery/dispatch/wake/pause-resume/retry/refresh responsibility — including terminal-transfer reacquisition — is defined ONCE, on `convergence_engine.TransferEngine`, and DELETED entirely (not stubbed, not aliased) from every lower class — CANON-001, checked per (class, method) against each lower class's own `__dict__`, not a hand-maintained list or a "safe because shadowed"/"safe because it raises" argument; quiescent equivalence hold consumed as an orthogonal fact proven consistent across `_process_request`/restart/wake/presentation, correctly BLOCKING completion (not just avoiding false `RESOLVING`) while unresolved, and never masking an independent genuine failure/cancellation (evaluated before the quiescent-hold fallback); unknown/known-zero/known-positive size is a real three-state fact (`transfers.models.SizeKnowledge`), so a `SUCCEEDED` observation with no known positive size can never complete or deliver) — see `UNIVERSAL_TRANSFER_CORE.md` | `test_canonical_parent_lifecycle.py`, `test_canonical_completion_truth.py`, `test_equivalence_retry_remediation.py`, `test_universal_hardening.py`, `test_universal_lifecycle.py`, `test_universal_parity.py`, `test_transfer_recovery_phase2.py`, `test_multi_mirror_general_http_convergence.py` |
...
- Exactly one parent-lifecycle decision owner (`transfers._repository_base.TransferRepository.aggregate_lifecycle`); no recovery, pause, or equivalence layer may independently persist a second, later parent-status write over the same decision.
- Exactly one recovery-decision/candidate-activation/dispatch-readiness/quiescent-wake/bulk-pause-resume/retry/refresh owner (`transfers.convergence_engine.TransferEngine`) for anything that decides policy or persists a recovery/lifecycle transition. None of `pause`/`resume`/`pause_all`/`resume_all`/`retry`/`_reacquire_transfer`/`_renew_source_parent`/`_refresh`/`_schedule_refresh`/`_recover_artifact` is defined at all on `transfers._engine_recovery.TransferEngine`, `transfers.engine.TransferEngine`, or `transfers._engine_base.TransferEngine` — not a working alternate, not a refusal stub, not an alias — checked directly against each class's own `__dict__`, per (class, method), not a hand-maintained list, a "safe because shadowed in production" argument, or a "safe because it raises" argument. `retry` owns both the operator-initiated decision and terminal-transfer reacquisition (`reacquire=True`, dispatching internally to `_reacquire_transfer`) as two branches of the one canonical implementation — `submit()` reaches the reacquire branch only when deduplication finds an existing COMPLETED/DELETED transfer, never unconditionally. A held equivalence request blocks parent completion (not merely `RESOLVING`) until resolved or released, but never masks a genuine, independent terminal failure or cancellation belonging to a different voting artifact/request — failure/cancellation are evaluated before the quiescent-hold fallback, which is the lowest-precedence branch.
- A completion verification decision consumes one canonical size-knowledge fact (`transfers.filesystem.size_knowledge`, `transfers.models.SizeKnowledge`); a candidate/executor total of `0` is absence of size knowledge, never affirmative zero-byte evidence, and can never complete or deliver. `KNOWN_ZERO` is representable but has no current producer (General HTTP/aria2/AllDebrid cannot distinguish an explicit zero from an absent field), enforced by a static source-scan regression.
...
## Final audit corrections (single authority / single owner)

| Contract | Permanent tests |
| --- | --- |
| Candidate identity: one publishable candidate per full source SHA, write-once `sha-<40-char-sha>` tag, existing-tag check before publish, fail-closed on revision mismatch, serialized same-SHA runs, no short-SHA identity in publication/qualification/promotion | `test_supply_chain_contract.py`, `test_release_promotion_contract.py` |
| Provider-neutral metrics (`debridpulse_*`); no `alldebrid_*` application family; no provider/executor claim in universal HELP text | `test_metrics_contract.py` |
| Canonical settings authority: real load → migrate → save → reload round trip; flat aliases are not fields, are not regenerated, never override canonical state; read-only compatibility projection distinct from authority | `test_canonical_settings_migration.py`, `test_settings_authority_contract.py`, `test_integration_configuration.py`, `test_executor_configuration_ownership.py` |
| The aria2 RPC client is constructed by the daemon owner (`executors.aria2.runtime.rpc_service`); no global settings lookup in the client | `test_settings_authority_contract.py`, `test_aria2_result_retention.py`, `test_v1111_aria2_security_boundary.py` |
| Frontend: one EventSource owner, one Activity Log owner, no topbar wrapper, no foreign-global or native-API replacement, canonical settings fields consumed directly | `test_ui_runtime_architecture_contract.py`, `test_ui_activity_log_page_contract.py`, `test_ui_presentation_ownership_contract.py`, `frontend/browser/consolidation-events.spec.js`, `topbar-concurrency.spec.js`, `settings-concurrency-persistence.spec.js`, `ui-presentation-owners.spec.js` |
| Engine/repository stacks contain no superseded override layer; presentation and recovery-state operations each have one owner | `test_engine_repository_ownership_contract.py`, `test_canonical_runtime_architecture.py` |
