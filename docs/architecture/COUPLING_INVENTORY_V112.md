...
| `services/rate_limit.py` | A/C | AllDebrid request pacing reads flat global provider settings. Provider implementation/configuration must own this pacing. (The setting itself is now `integrations.alldebrid.options.rate_limit_per_minute`; the flat name is migration input only.) |
...
| `core/config.py`, `config_validator.py` | C/D | **Resolved (final audit).** `integrations.<id>`, `transfer_policy` and `execution_runtime_limits` are the only persisted/runtime authorities; the flat aliases are not fields of `AppSettings` and are read only by the one load-time migration boundary (`integrations.configuration.migrate_legacy_settings`). `GET /settings` derives the historical names read-only (`api/legacy_settings_view.py`). `Aria2Service` receives daemon ownership by injection. Guarded by `test_canonical_settings_migration.py`, `test_settings_authority_contract.py`, `test_integration_configuration.py`. |
...
| `services/stats.py`, events and metrics | B/C/D | **Resolved (final audit).** `/api/metrics` now publishes universal transfer/scheduler state under the provider-neutral `debridpulse_*` namespace; the `alldebrid_*` application family is removed with no dual-publish shim (v1.0.12 is unreleased, so the old names are not preserved). Guarded by `test_metrics_contract.py`. Source distinctions in stats aggregation are unchanged. |
