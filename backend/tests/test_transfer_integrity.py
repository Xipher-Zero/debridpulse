...
def test_runtime_service_root_uses_guarded_integrity_engine():
    ...
    retry_guard = (services / "direct_link_retry_guard.py").read_text()
    result_guard = (services / "direct_link_result_guard.py").read_text()
    lifecycle_guard = (services / "transfer_runtime_guard.py").read_text()
    assert "from services.direct_link_retry_guard import manager as engine" in root
    assert "from services.manager_v2 import manager as engine" not in root
    assert "class DirectLinkRetryGuardManager(DirectLinkResultGuardManager)" in retry_guard
    assert "from services.direct_link_result_guard import DirectLinkResultGuardManager" in retry_guard
