...
def test_authentication_settings_are_owned_by_clean_settings_runtime():
    ...
    # ...and no page-level save of any kind survives anywhere on Settings,
    # so there is no longer a set of tabs that are exempt from one.
    assert "FIELD_BOUNDARY_TABS" not in module
    assert "nonAuthPayload" not in module
