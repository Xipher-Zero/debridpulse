...
@pytest.mark.asyncio
async def test_automatic_purge_preserves_bounded_result_state(monkeypatch):
    ...
    assert result == {
        "skipped": True,
        "reason": "bounded aria2 result state is operator-visible",
    }
    service._call.assert_not_awaited()


@pytest.mark.asyncio
async def test_explicit_force_purge_remains_available(monkeypatch):
    import executors.aria2.client as aria2_module
