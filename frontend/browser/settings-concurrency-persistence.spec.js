...
test('configured scheduler denominator wins over later telemetry refresh',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{
   settingsData={integrations:{aria2:{options:{mode:'builtin'}}},transfer_policy:{max_concurrent_executions:6},execution_runtime_limits:{max_download_bytes_per_second:0},paused:false};
   ...
   try{await loadAria2SpeedLimit();updateAria2TopbarBadge({active:3,liveBps:1024});return{before,after:{canonical:settingsData.transfer_policy.max_concurrent_executions,flat:['max_concurrent_downloads','aria2_max_active_downloads'].filter(key=>key in settingsData),active:document.getElementById('aria2-badge-active')?.textContent,max:document.getElementById('aria2-badge-max')?.textContent}};}finally{api=originalApi;}
 });
