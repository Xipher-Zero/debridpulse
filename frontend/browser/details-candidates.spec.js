...
async function openCandidates(page, holder, handler) {
  ...
  await page.locator('tr[data-artifact-id="502"] .dp-detail-candidate-disclosure').click();
  await expect(page.locator('tr[data-candidate-owner="502"]')).toBeVisible();
}
...
test('manual switch is backend authoritative and keeps Details usable', async ({page}) => {
  ...
  const panel=page.locator('tr[data-candidate-owner="502"]');
  ...
  await expect(panel.locator('.dp-detail-candidate-item').first().locator('.dp-detail-candidate-active')).toHaveText('ACTIVE');
  await expect(page.locator('tr[data-candidate-owner="502"] .dp-detail-candidate-item').nth(1).locator('.dp-detail-candidate-active')).toHaveText('ACTIVE');
  await expect(page.locator('.toast')).toContainText('GF030926-M2SP-RN.rar file source switched to megaup.net');
  const disclosure=page.locator('tr[data-artifact-id="502"] .dp-detail-candidate-disclosure');
  await expect(disclosure).toBeFocused();
  await disclosure.click();
  await expect(page.locator('tr[data-candidate-owner="502"]')).toHaveCount(0);
  await disclosure.click();
  await expect(page.locator('tr[data-candidate-owner="502"]')).toBeVisible();
});
...
test('structured server rejection stays truthful and does not move ACTIVE', async ({page}) => {
  ...
  await page.locator('tr[data-candidate-owner="502"] .dp-detail-candidate-switch').click();
...
test('in-flight suppression prevents duplicate candidate POSTs', async ({page}) => {
  ...
  const button=page.locator('tr[data-candidate-owner="502"] .dp-detail-candidate-switch');
