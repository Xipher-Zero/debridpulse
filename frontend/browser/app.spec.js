...
test('Download speed cap menu exposes the corrected preset grid and submits 30 MB/s as 31457280', async ({ page }) => {
  await isolateExternalFonts(page);
  const runtime = observeRuntime(page);
  await page.goto('/');
  ...
  expect(labels).not.toContain('2 MB/s');

  const values = await presets.evaluateAll(nodes => nodes.map(node => node.dataset.capBps));
