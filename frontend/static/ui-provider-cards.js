...
(function(){
  ...
  function decorate(enable){const id=String(enable.dataset.integrationEnabled||'');const integration=integrations()[id];const card=enable.closest('.dp-settings-provider-card');if(!integration||!card)return;addPremium(card,integration);applyGroupTitle(card,integration);if(!integration.presentation?.premium||card.dataset.dpProviderStateOwner==='1')return;
    ...
    const update=()=>{const p=statusPresentation(enable.checked,!!integration.configured);status.textContent=p.text;status.dataset.tone=p.tone;status.hidden=!p.text;};
