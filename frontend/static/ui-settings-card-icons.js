...
(function () {
  ...
  let scheduled=false;
  function normalizedTitle(title){const clone=title.cloneNode(true); clone.querySelectorAll('.dp-settings-inner-card-icon,[data-dp-settings-replaced-icon="1"]').forEach(n=>n.remove()); return String(clone.textContent||'').replace(/\s+/g,' ').trim();}
  function decorate(title,section,src){if(title.querySelector(':scope > .dp-settings-inner-card-icon'))return; Array.from(title.children).forEach(child=>{if(child.matches('[aria-hidden="true"]')&&(child.querySelector('img,svg')||child.matches('.dp-settings-download-engine-icon,.dp-settings-aria2-live-icon'))){child.dataset.dpSettingsReplacedIcon='1'; child.classList.add('dp-settings-replaced-legacy-icon');}}); title.classList.add('dp-settings-card-title--with-icon','dp-settings-inner-card-title'); title.dataset.dpSettingsIconSection=section; const frame=document.createElement('span'); frame.className='dp-settings-inner-card-icon'; frame.setAttribute('aria-hidden','true'); frame.dataset.section=section; const img=document.createElement('img'); img.src=src; img.alt=''; img.decoding='async'; frame.appendChild(img); title.prepend(frame);}
  function apply(){const view=document.getElementById('view-settings'); if(!view)return; view.querySelectorAll('.card-header .card-title').forEach(title=>{const def=ICONS[normalizedTitle(title)]; if(def)decorate(title,def[0],def[1]);});}
