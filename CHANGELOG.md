...
pipeline with a provider-independent **Universal Transfer Core** that owns transfer identity,
lifecycle, policy, routing, recovery, provenance and canonical result authority, and reduces
providers and executors to independent implementations of neutral contracts. AllDebrid and
General HTTP(S) are the production providers; aria2 is the production executor.

### Core separation and ownership

- One universal lifecycle owner replaces the inherited managers, gateways and captured-method coordinators. **Providers** own applicability, provider-native resolution, authentication and resource facts, and provider observations. **Executors** own execution submission, observation, control and native execution facts. **The Universal Transfer Core** owns everything else: lifecycle, policy, routing decisions, retry and recovery, reconciliation, pause/resume/cancel/delete semantics, completion authority, provenance, restart behavior and canonical result authority.
- Native protocol handling and native execution sit behind independent contracts. Native errors become canonical, multidimensional failures at the adapter boundary; unknown and security failures never trigger blind retries. Unsafe native URL/path adoption and hidden native retries were removed — an existing payload requires verified possession, and a terminal parent cannot release a path while execution remains uncertain.
- Accumulated cross-layer ownership was collapsed into single canonical owners. `transfers.convergence_engine.TransferEngine` is the sole owner of semantic recovery and control operations (pause, resume, pause-all, resume-all, retry, refresh, candidate activation); every lower-layer alternate implementation was deleted rather than stubbed, and the cross-module monkeypatch seam between the engine modules is gone. Recovery and repository correctness no longer depends on method-resolution override order.
- `integrations.<id>`, `transfer_policy` and `execution_runtime_limits` are the only persisted settings authorities; the flat legacy names are migration input and a read-only projection. Global processing pause is durable operational state, not a setting. Metrics are published once, under `debridpulse_*`.

### Canonicalization and consolidation

- Durable canonical identities and relationships replace inference from URL and name shape: transfer requests, provider resources, canonical artifacts, candidates, candidate bindings, origins and provenance, resolution and execution attempts, and consolidation records are all persisted facts. **URL shape is never a substitute for durable provenance or history.**
- Canonical candidate identity is immutable. Manual and automatic candidate switching are one canonical operation (`transfers.candidate_activation.activate_candidate`) that owns writer retirement, partial/resume policy, provenance and a truthful post-commit acknowledgement as part of the operation itself.
- Equivalence between independently sourced copies of one file is decided by affirmative evidence, protocol-neutrally. An unknown reported size means unknown, not proof of difference; a short response that the declared expected size makes implausible is reported as an incomplete representation rather than as fabricated certainty; two known incompatible sizes remain a cheap rejection. Retry-budget exhaustion during a proof never authorizes an independent writer — identity stays unresolved and held, with no competing writer and no hot loop, until evidence resolves or releases it.
- An empty canonical set no longer grants sole seed authority to whichever sibling reaches materialization first: a bootstrap admission barrier stops a structurally or transiently unsampleable mirror from starving a viable one.
- Same-transfer canonical membership is discoverable through candidate binding and origin provenance, not only through consolidation rows. Parent lifecycle voting reflects logical delivery obligations: a failed row whose own logical delivery slot is already satisfied by a different, completed canonical artifact no longer poisons an otherwise-complete parent, while a genuinely independent failed artifact still votes.
- Intra-transfer and inter-transfer consolidation share one model. One batch submission remains one durable transfer owning independent root requests, and equivalence keys on request identity rather than transfer identity, so same-transfer siblings and genuinely separate transfers converge through the same path.
- Deleting a transfer permanently retires its dedupe identity, and provider-resource identity is split into a transfer-independent canonical resource and a per-transfer binding generation. An identical native resource returned for a re-upload can therefore never alias one transfer's manifests or selections into another, and a retired predecessor's provenance and outstanding cleanup stay scoped to that historical transfer.
- Route and failover history is durable and provider-neutral. Details renders the logical source route from each attempt's own recorded result — never from a provider-issued delivery URL — and reports unknown identity rather than guessing when an attempt or candidate is ambiguous. Provider-native cache evidence is a neutral `CachePresence` fact mapped once at the adapter boundary and never inferred from later readiness, status codes, speed or endpoint host; no cache-aware provider selection is introduced.

### Universal lifecycle and recovery

- One common lifecycle machinery owns retry, resume, startup reconciliation, provider recovery, executor recovery, pause and resume, cancellation and deletion, capacity waits, storage containment and recovery, input-required/authentication continuation, restart and crash reconciliation, and completion and failure authority.
- Current recovery state is state, not an event log: one row per artifact in `artifact_recovery_state`, updated in place and migrated additively and idempotently from the legacy event history. Historical audit facts never double as current policy.
- Parent aggregation reads and writes inside one coherent fenced transaction, closing the torn-snapshot race where a stale decision could overwrite an already-committed outcome. Claim-lost detach and park mutations are fenced atomically, so a recovery owner whose claim was superseded cannot commit a mutation.
- Provider cleanup claims are durable leases with per-call heartbeats rather than bare booleans, so a crash, a cancellation or a post-claim exception can no longer strand a claim and fence a fresh re-add of the same source indefinitely. A fresh re-add is admitted immediately and held as ordinary waiting state behind a cleanup fence — never as a recovery failure.
- Resolution admission is bounded and fair, a running resolution cycle admits work that becomes runnable during that cycle, and a candidate making no progress is refreshed by policy rather than by a second scheduler.
- Deferred intake, durable pause intent, capacity-aware resume, stable retry paths, completed siblings, mirror failover, owned cleanup and secure archive post-processing are preserved. Canonical settlement transactionally retires stale pause intent and input-required side state at every path that can settle a parent transfer.
- Neutral non-terminal `INPUT_REQUIRED`, with an initial `AUTH_REQUIRED` challenge, carries a durable non-secret challenge identity and generation plus transient one-transfer credential delivery, and continues the same transfer through provider or executor. `username_password` and `username_private_key` descriptors are defined; a private-key passphrase is optional and is not a separate authentication method. Authentication waits preserve retry budgets and ordinary capacity, survive restart without persisting secrets, and remain subordinate to pause, cancel, delete and capacity control. Saved credentials are deliberately out of scope.

### Provider-neutral routing and applicability

- Provider applicability parsing and normalization are centralized and provider-neutral, with canonical scheme/host/port handling, explicit exact-host and domain-scope claims and deliberate IDNA/IP semantics, and no destructive rewriting of transfer endpoints.
- `SPECIALIZED` and `GENERIC` applicability: a matching specialized claim suppresses generic URL handlers, and neutral preferred-provider, priority and stable-ID ordering then decides within the surviving class. When no specialized claim legitimately wins, an eligible generic provider handles the request. When no eligible provider remains, routing returns a canonical non-retryable `UNSUPPORTED_REQUEST` without contacting any provider or executor.
- AllDebrid owns its own dynamic supported-host inventory through its existing paced client: provider-local domain, regex, availability and quota interpretation, opaque last-known-good persistence, a maintenance-only approximately 24-hour refresh, and recovery across restart and disable/re-enable. Ordinary link, magnet and torrent submission never refreshes host inventory. General HTTP & HTTPS contributes generic `http`/`https` applicability; magnet and torrent remain provider-declared static request-type routing.
- Routing truth is independent of timing, arrival order and readiness. Provider runtime payload interpretation, freshness and host knowledge stay inside the provider, and the failure classifier stays provider-neutral.

### Execution and material truth

- aria2 is the current execution engine, behind the executor contract. **It executes and observes native work; it does not own the transfer lifecycle, universal provider routing or universal retry policy.** Safe external-aria2 ownership boundaries are preserved: DebridPulse tracks the downloads it owns and does not change daemon-wide bandwidth limits, rewrite global configuration, purge global result history or control unrelated GIDs.
- Completion is verified, not assumed. A `SUCCEEDED` execution with no known positive size can never be treated as a completed zero-byte payload: `download_files.size_knowledge` records durable size truth and `SizeKnowledge` makes known-positive, known-zero and unknown three distinct provable states. Completion is gated on affirmative known-zero evidence, so **unproven zero-byte material fails closed**.
- Size reconciliation has one owner, with a provisional writer for bootstrap and bounded drift resolved at completion, rather than per-path size overrides.
- **Deferred — `STAGED-MATERIALIZATION-001`.** 1.0.12 protects against unproven zero-byte material by *rejecting and retiring* it, not by preventing its materialization. The executor still dispatches at the canonical target path, so a zero-byte file can exist transiently before the core retires it. Staged execution-owned materialization with atomic promotion is **not** implemented in 1.0.12 and is carried forward to 1.0.13.

### Operator-facing truth and scalability

- Provider and executor provenance is durable, and it is what Recent Activity, Downloads and Details report: current and final provider plus ordered route history, read from persisted facts rather than inferred from a URL.
- Details presents the canonical object rather than only the physical artifacts, with a presentation/artifact identity split for file rows, and the transfer's materialized path chosen from the durable-binding-filtered voting set rather than by row-creation order.
- Universal torrent and magnet file selection is core-owned. A provider-neutral `FILE_MANIFEST` capability lets a capable provider report a neutral early manifest (safe name, relative path and expected size only) at the adapter boundary; the core owns every selection decision — ALL-versus-explicit-subset policy with ALL as the default, the decision window anchored to the arrival of the first actionable multi-file manifest, durable per-(request, provider-resource binding) selection generations that a re-resolution never inherits, stale-manifest rejection, and fail-closed executable-manifest reconciliation that never broadens back to ALL. Submission intent is explicit (`selection_mode`), so a headless or API caller keeps ALL and never becomes dependent on a browser. Provider acquisition and resolution start immediately and never wait on the browser; only local materialization and executor dispatch are held, and Confirm or Close releases that hold immediately instead of waiting out the deadline. The Confirm-versus-materialization race is serialized by durable SQLite, never by an in-memory lock.
- Canonical candidate and source presentation is shared across Dashboard, Downloads and Details: per-artifact candidates, transfer-level common-source groups computed as a strict canonical-host intersection over remaining work, and a contextual action scope. Group membership, group actionability and group-active state stay three independent facts, a completed file never vetoes convergence for the files still working, and a switch that cannot be made is reported truthfully rather than rolled back synthetically.
- Operational reads stay bounded projections. Downloads and Recent Activity are served by one page-scoped SQL read with constant round-trips regardless of page size and no per-row presentation calls; the per-artifact recovery lookup became an indexed set-oriented join (a 700-file torrent's page load went from roughly 2 s to roughly 4 ms on a realistic fixture); Recent Activity priority ordering acquires candidates by indexed status seek with a fixed-cap recovery net, so cost is bounded by page size rather than by history size.
- Waiting and recovery states are presented truthfully and identically on every surface through one effective-presentation owner — including a distinct "Waiting for provider" while a provider resource is still being prepared. A settled decision never renders as an active one, and active transfer progress refreshes every second.
- The browser application has one bounded owner per visible behavior. Downloads has a single load and render owner, Settings a single markup owner, the dialog shell a single owner with a defined focus-restoration boundary; the EventSource, topbar and `window.fetch` monkeypatches and the test-only compatibility globals are gone. The stylesheet graph is one import graph: the versioned structural scoping substrate, the catch-all foundation stylesheet and the correction-layer stylesheets were physically removed rather than reflattened.

### Security and release hardening

- Authentication: intentional no-auth mode, Username & Password with HTTP Basic for REST clients, provider-neutral OIDC (Authorization Code + PKCE), optional bearer API tokens, CSRF, sessions, lockout and throttling, and a transition state machine that refuses to disable the password path until a real OIDC sign-in has succeeded.
- Archive extraction safety: isolated staging, validated commit into the download tree with no-clobber semantics, per-archive file-count, expanded-size and compression-ratio limits, and exact outer decoders feeding the validated TAR path instead of broad parser selection.
- Dependency and license closure: runtime dependencies are exactly pinned and hash-pinned, installed with `--require-hashes`, and the lock is enforced byte-exact against `licenses/python-runtime.json` and `docs/DEPENDENCY_LICENSES.md`.
- The base image is pinned by verified multi-architecture manifest digest, with named, scoped package refreshes replacing the blanket upgrade; the policy and its feedback loop are documented in `docs/SUPPLY_CHAIN_POLICY.md`.
- The image is the artifact. It is built once immutably and tagged by the full 40-character source SHA (write-once, existence-checked, fail-closed on revision mismatch), then scanned and runtime-qualified on `linux/amd64` and `linux/arm64` with signed in-registry attestations. Mutable tags are only ever re-pointed at a digest that has already passed every required gate; nothing is rebuilt at promotion.
- Qualification determinism: each suite runs once and a single classifier decides candidate-versus-anchor on bounded evidence, with an unresolvable anchor failing closed and a known-flake registry that holds zero active entries and is not a skip list. See [`docs/QUALIFICATION_DETERMINISM.md`](docs/QUALIFICATION_DETERMINISM.md).
- Database maintenance correctness: the backup and wipe inventory covers every current table, and wipe order is derived from the live foreign-key graph rather than from a hand-maintained list.

### Deferred to 1.0.13

These are **1.0.13 expansion work, not unmet 1.0.12 release requirements**:

- `STAGED-MATERIALIZATION-001` — staged execution-owned materialization with atomic promotion;
- FTP, SFTP, SCP, rsync, WebDAV and NZB/SAB transports;
- additional debrid providers, and additional executor implementations;
- broader provider and executor expansion, and the executor/settings UI expansion that several real executor shapes would justify.

No implementation order is promised beyond that. New provider, executor or protocol work in
1.0.13 must trigger a fresh dependency and license review if it introduces dependencies,
copied or derived code, or other attribution obligations.
